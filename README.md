...
***
## 트러블슈팅 🛠️ <br>
### 1️⃣ 템플릿 네트워크 설정 문제로 IP가 적용되지 않음

VM 배포 시 Guest Customization으로 IP를 설정했지만 VM에 적용되지 않는 문제가 발생했습니다.
<br><br> 템플릿 내부에 기존 네트워크 설정이 남아 있어 충돌이 발생하고 있었고, 템플릿에서 해당 설정을 정리한 뒤 다시 생성하자 IP가 정상적으로 적용되었습니다.

### 2️⃣ VM 생성 직후 SSH 통신이 되지 않는 문제

VM 생성은 성공했지만 Guest OS 부팅과 네트워크 초기화가 완료되기 전에 SSH 연결을 시도하면서 접속이 실패했습니다.
<br><br>이를 통해 VM 생성 완료와 접속 가능 시점은 다르다는 점을 확인했고, 이후에는 IP 확인 및 SSH 가능 여부를 고려한 연결 흐름이 필요하다는 것을 알게 되었습니다.

### 3️⃣ 환경 변수 하드코딩으로 유지보수가 어려웠던 문제

초기 구현 과정에서 vCenter와 Guacamole 설정 일부를 코드에 하드코딩해두었습니다.
<br><br>이 때문에 IP나 계정 정보 하나만 바꾸려고 해도 여러 파일을 수정해야 하는 문제가 발생했고, 이후에는 application.yml과 @ConfigurationProperties를 통해 설정값을 분리하는 방식의 중요성을 느끼게 되었습니다.
